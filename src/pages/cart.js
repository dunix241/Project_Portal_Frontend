...
import { useEditCartDetailsMutation, useListCartDetailsQuery } from '../agent/cartApiSlice';
...
function Page() {
  ...
  const totalPrice = useMemo(() => {
    return data?.reduce((prev, current) => prev + current.product.price, 0) || 0
  }, [data])
            ...
            <Paper sx={{p: 3}}>
              ...
              <Stack>
                {data?.map(cartItem => {
                  ...
                  const {id, price, discount, stocks} = cartItem.product
                  return (
